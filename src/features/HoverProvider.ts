...
export class HoverProvider implements vscode.HoverProvider {
  provideHover(document: vscode.TextDocument, position: vscode.Position): vscode.ProviderResult<vscode.Hover> {
    ...
    const entry = decorator.entries.find(item => {
      const startPos = document.positionAt(decorator.offsetBase + item.pos - 1);
      const endPos = document.positionAt(decorator.offsetBase + item.pos + item.nameInfo.text.length - 1);
    ...
    if (entry) {
      ...
      const entryName = entry.nameInfo.text;
